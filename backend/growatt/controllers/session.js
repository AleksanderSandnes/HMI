...
class Session {
     ...
     */
    constructor(username, password, server = "https://server.growatt.com/", headers) {
    ...
    async login() {
        return new Promise((resolve, reject) => {
            ...
            delete this.cookie;
            const params = new Url.URLSearchParams({ account: this.username, password: this.password, validateCode: '' });

            this.axios
                .post(this.getUrl('login'), params.toString(), { headers: this.headers })
                .then(res => {
                    if (res.data && res.data.result && res.data.result === 1) // Correct request
                    {
 ...
 */
    ...
    async checkCookieValidity() {
        if (!this.isConnected || new Date(this.cookieExpiration).getTime() < new Date().getTime()) {
            console.log("Session closed or cookie expired. New login will be performed...");
            await this.login();
        }
    }
}

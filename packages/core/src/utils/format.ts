// Shared numeric coercion / display helpers for the dashboard widgets.
// Sensor payloads arrive loosely-typed (numbers, null, NaN), so every tile
// needs the same null-safe coercion + "—" fallback — centralised here so web
// (apps/web) and mobile (apps/mobile) share one implementation instead of
// re-declaring it per app.

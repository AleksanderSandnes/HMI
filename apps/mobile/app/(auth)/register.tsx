...
> = {
  0: { icon: 'person-add', gradient: 'accent', title: 'Create account', subtitle: 'Join your energy monitoring dashboard' },
  1: { icon: 'sunny', gradient: 'energy', title: 'Connect Growatt', subtitle: 'Optional — add your solar credentials' },
  2: { icon: 'partly-sunny', gradient: 'solar', title: 'Connect Weather.com', subtitle: 'Optional — add your weather station' },
};
    ...
    >
      ...
      >
        <GlassCard strong elevated className="w-full max-w-[460px] p-8">
          ...
          {step === 0 ? (
            <View>
              <Field label="EMAIL ADDRESS" icon={mail} keyboardType="email-address" autoCapitalize="none" placeholder="you@domain.com" value={account.email} onChangeText={(t) => setAccount((s) => ({ ...s, email: t }))} error={accountErrors.email} editable={!saving} />
              <Field label="USERNAME" icon={user} autoCapitalize="none" placeholder="Choose a username" value={account.username} onChangeText={(t) => setAccount((s) => ({ ...s, username: t }))} error={accountErrors.username} editable={!saving} />
              <Field label="PASSWORD" icon={lock} secure placeholder="Create a secure password" value={account.password} onChangeText={(t) => setAccount((s) => ({ ...s, password: t }))} error={accountErrors.password} editable={!saving} />
              <Field label="CONFIRM PASSWORD" icon={lock} secure placeholder="Re-enter your password" value={account.confirmPassword} onChangeText={(t) => setAccount((s) => ({ ...s, confirmPassword: t }))} error={accountErrors.confirmPassword} editable={!saving} />
              <Button label="Create account" icon={arrow} onPress={handleCreateAccount} loading={saving} className="mt-1.5 w-full" />
            </View>
          ) : null}

          {step === 1 ? (
            <View>
              <Field label="ACCOUNT (EMAIL)" icon={mail} keyboardType="email-address" autoCapitalize="none" placeholder="your-email@domain.com" value={growatt.email} onChangeText={(t) => setGrowatt((s) => ({ ...s, email: t }))} editable={!saving} />
              <Field label="PASSWORD" icon={key} secure placeholder="Enter your Growatt password" value={growatt.password} onChangeText={(t) => setGrowatt((s) => ({ ...s, password: t }))} editable={!saving} />
              ...
              <View className="flex-row gap-3">
                <Button label="Skip" variant="ghost" onPress={() => { setStepError(null); setStep(2); }} className="flex-1" />
                <Button label="Continue" icon={arrow} gradient="energy" onPress={handleSaveGrowatt} loading={saving} className="flex-1" />
              </View>
            </View>
          ) : null}
          ...
          {step === 2 ? (
            <View>
              <Field label="WEATHER STATION ID" icon={pin} autoCapitalize="characters" placeholder="e.g. ISANDN24" hint="Find your local station ID at weather.com/weather/map" value={weather.stationId} onChangeText={(t) => setWeather((s) => ({ ...s, stationId: t }))} editable={!saving} />
              <Field label="API KEY" icon={key} secure placeholder="Enter your Weather.com API key" hint="Get your API key from the weather.com developer portal" value={weather.apiKey} onChangeText={(t) => setWeather((s) => ({ ...s, apiKey: t }))} editable={!saving} />
              <Text className="mb-4 mt-0.5 text-center text-[12.5px] font-medium leading-[18px] text-text-muted">
                Optional — you can add or change this anytime in Settings.
              </Text>
              <View className="flex-row gap-3">
                <Button label="Back" variant="ghost" onPress={() => { setStepError(null); setStep(1); }} className="flex-1" />

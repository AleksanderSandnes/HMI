...
const METRICS: MetricMeta[] = [
  { key: 'temperature', label: 'Temp', icon: io('thermometer'), unit: '°C', title: 'Temperature', accent: '#fb7185', series: [{ label: 'Temperature', color: '#fb7185' }, { label: 'Dew point', color: '#34d399' }] },
  { key: 'windSpeed', label: 'Wind', icon: (c, s) => <MaterialCommunityIcons name="weather-windy" color={c} size={s} />, unit: 'km/h', title: 'Wind', accent: '#60a5fa', series: [{ label: 'Wind speed', color: '#60a5fa' }, { label: 'Wind gust', color: '#fbbf24' }] },
  { key: 'precip', label: 'Rain', icon: io('rainy'), unit: 'mm', title: 'Precipitation', accent: '#38bdf8', series: [{ label: 'Accum. total', color: '#38bdf8' }, { label: 'Rate', color: '#34d399' }] },
  { key: 'humidity', label: 'Humidity', icon: io('water'), unit: '%', title: 'Humidity', accent: '#22d3ee', series: [{ label: 'Humidity', color: '#22d3ee' }] },
  { key: 'pressure', label: 'Press', icon: io('speedometer'), unit: 'hPa', title: 'Pressure', accent: '#a78bfa', series: [{ label: 'Pressure', color: '#a78bfa' }] },
  { key: 'solarRadiation', label: 'Solar', icon: io('sunny'), unit: 'W/m²', title: 'Solar radiation', accent: '#fbbf24', series: [{ label: 'Solar radiation', color: '#fbbf24' }] },
  { key: 'uvIndex', label: 'UV', icon: io('sunny-outline'), unit: 'UV', title: 'UV index', accent: '#c084fc', series: [{ label: 'UV index', color: '#c084fc' }] },
];
...
export default function Weather() {
  ...
  const series = useMemo(() => buildWeatherSeries(observations ?? [], dataType, timespan), [observations, dataType, timespan]);
  const bands = useMemo(() => buildWeatherDailyBands(observations ?? [], dataType), [observations, dataType]);
